class Ball {
  ...
  bounceEdge() {
    if (this.location.x + this.radius + 20 >= width) { // Check right edge
      this.reverse('x')
    } else if(this.location.x - this.radius + 20 <= 0) { // Check left edge
      this.reverse('x')
    } else if(this.location.y - this.radius + 20 <= 0) { // Check the top
      this.reverse('y')
    }
  }
  ...
  display() {
    fill(this.color)
    //ellipse()
    image(img, this.location.x, this.location.y, this.size, this.size)
  }
